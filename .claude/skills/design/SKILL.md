...
- Shimmer: `::before` slides `left: -100% → 100%` via `transition: left 0.5s ease-out`
...
- Button press: `active:scale-[0.98]` at `transition-duration: 100ms`

Never use `animate-bounce`, `animate-ping`, or rapid flashing. Motion is physical and deliberate.
...
- ❌ `text-black` — use `text-gray-900` or `text-[#1E240A]`
- ❌ Bright/saturated accents — palette is muted, natural, earthy
- ❌ Background gradients (except footer radial texture at opacity-5)

### Typography

- ❌ `font-bold` on headings — 500 or 600 only
- ❌ Inter for headings — always Playfair Display
- ❌ `text-xs` for body — minimum `text-sm`
- ❌ Line-height below 1.6 on body text
- ❌ Random tracking — `tracking-[0.2em]` for labels, `tracking-wider` for buttons only

### Buttons & Shape

- ❌ `rounded-full` pill buttons — use `rounded` (4px)
- ❌ `rounded-xl` or `rounded-2xl` anywhere
- ❌ Shadows not matching brand: `hover:shadow-blue-500/20`
- ❌ Buttons without hover states

### Layout

- ❌ Full-width sections without `max-w-7xl mx-auto`
- ❌ Off-grid padding: `p-3`, `p-5`, `p-7`
- ❌ `gap-3` in major grids — `gap-6` minimum
- ❌ Inline styles for spacing

### General

- ❌ Emoji in UI (admin/debug pages excepted)
- ❌ Lorem ipsum
- ❌ Placeholder gray boxes instead of real empty states
- ❌ `text-center` on long body paragraphs (section headers only)
- ❌ `shadow-xl` or `shadow-2xl` — use `shadow-sm` / `shadow-md`
- ❌ Borders everywhere — prefer space and shadow over borders
...
- Logo: SVG from `/assets/logos/` — `logo-white.svg` and `logo-mark-white.svg`
- Never `<img>` in production components — always `next/image` or `SignedImage`
